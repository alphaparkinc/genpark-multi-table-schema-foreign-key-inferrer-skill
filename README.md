# genpark-multi-table-schema-foreign-key-inferrer-skill
...
**genpark-multi-table-schema-foreign-key-inferrer-skill** is a deterministic, zero-dependency Python skill engineered for autonomous spreadsheet data sanitization, reverse CTE SQL synthesis, and executive BI analytics.

> **Executive Capability**: Relational database foreign key & ERD path inferrer (Dataherald)
...
result = client.infer_relational_schema()
print(result)
```
...
```json
{
  "mcpServers": {
    "genpark-multi-table-schema-foreign-key-inferrer-skill": {
      "command": "python",
      "args": ["/path/to/genpark-multi-table-schema-foreign-key-inferrer-skill/mcp_server.py"]
    }
  }
}
```
